...
public class Ballet : MonoBehaviour
{
    ...
    {
        // 速度変更
        currentVelocity = Common.GetTargetVelocity(direction, weapon.MaxSpeed, false);
    }
    ...
    void IgnoreCollision()
    {
        ...
        {
            Transform machineTransform = obj.transform.Find(MachineConst.MACHINE);
            if (machineTransform != null)
            {
                Physics2D.IgnoreCollision(GetComponent<Collider2D>(), machineTransform.GetComponent<Collider2D>());

                // 盾を持っている場合は盾も無効
                Transform shieldTransform = Common.FindObjectRecursively(machineTransform, MachineConst.SHIELD);
