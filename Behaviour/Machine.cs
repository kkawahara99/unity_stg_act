...
//         string pilotName = pilot.model.PilotName;
...
//             // ダッシュ時はスピード+10、加速2倍
//             int dashSpeed = Calculator.GetDashSpeed(spd);
//             int dashAccel = Calculator.GetDashAccel(pilot.Acceleration);

//             // 目標速度を計算
//             Vector2 targetVelocity = Util.GetTargetVelocity(direction, dashSpeed, isDefence);

//             // 加速度に応じた速度の適用
//             currentVelocity = Util.GetCurrentVelocity(currentVelocity, targetVelocity, dashAccel);
...
//         {
//             // 目標速度を計算
//             Vector2 targetVelocity = Util.GetTargetVelocity(direction, spd, isDefence);

//             // 速度変更
//             currentVelocity = Util.GetCurrentVelocity(currentVelocity, targetVelocity, pilot.Acceleration);
...
//         float swingSpeed = Calculator.CalcSwingSpeed(pilot.Slashability);
//         while(upRotation < -1f)
//         {
...
//         float waitTime = Calculator.CalcWaitTime(pilot.Slashability);
//         yield return new WaitForSeconds(waitTime);
...
//             {
//                 // チャージ速度に応じて PPを増やす
//                 float chargeSpeed = Calculator.calcChargeSpeed(propellantPoint);
